...
$sql = "select pd.timestamp, pd.parkingCapacity, pd.vacantSpaces as vacantspaces, pd.open, pd.full, l.name from parkingdata pd left join locations l on l.identifier = pd.identifier where pd.timestamp >= date_add(now(), interval - 30 day) and pd.identifier='$uuid' order by pd.timestamp";
