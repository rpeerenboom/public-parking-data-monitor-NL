...
const realJson = [
...

  {
    ...
    "dynamicDataUrl": "//opd.it-t.nl/Data/parkingdata/v1/amsterdam/dynamic/900000002_parkinglocation.json",
    "limitedAccess": false,
  ...
  },
  {
    ...
    "dynamicDataUrl": "//opd.it-t.nl/Data/parkingdata/v1/amersfoort/dynamic/fe644297-9f7e-4a47-9ed6-e0804788a523.json",
    "limitedAccess": false,
  ...
  },
  {
    ...
    "dynamicDataUrl": "//opd.it-t.nl/data/parkingdata/v1/veere/dynamic/8f1b501e-4681-494b-bbf4-39c00a41e129.json",
    "limitedAccess": false,
    "geoLocation": null
  }
...
class Dashboard extends Component {
  ...
  generateRow(tbody, columns, node) {
    ...
    for (let j = 0; j < columns.length; j++) {
      ...
      if (columns[j] === "name") {
        ...
        classN += " heatCell"//colored heatcell
        // get json
        fetch(/*node["staticDataUrl"]*/ "//api.openparking.nl/parkingdata/request/staticurl/" + node["uuid"])
  ...
  /*componentDidUpdate(){
    ...
    let name = this.state.treemapData["name"]
        ...
        let sub = level  + name
        let url = "//api.openparking.nl/parkingdata/" + summaryStr + sub + "/?" + this.getParameters()
        console.log(url)
    ...
    let thiss = this
        ...
        .then(json => {
          ...
          componentDidUpdate(nextProps){
            ...
            let extras = this.props.filters.extras
            if (extras.indexOf("private") >= 0 && extras.indexOf("public") >= 0) {
              //noth checked, nothing extra needed
            } else if (extras.indexOf("private") >= 0 && extras.indexOf("public") === -1) {
              // only private -> &private=true
              url += "&private=true"
            } else if (extras.indexOf("private") === -1 && extras.indexOf("public") >= 0) {
              // only public -> &private=false
              url += "&private=false"
            } else {
              // strange? we want nothing??
              url = "//api.openparking.nl/parkingdata/summary/country/nl/"
            }
  ...
  async setAllParkings(tbody, column) {
    ...
    for (let i = 1; i < 0; i++) {
      let url = "//api.openparking.nl/parkingdata"
      let resultJson = null
  ...
  setNodesTreemapZoom(req) {
    ...
    let thiss = this
    let url = "//api.openparking.nl/parkingdata/" + req + "?" + this.getParameters()
    fetch(url)
      .then(response => response.json())
      .then(json => {
  ...
  setNodesTreemap() {
    ...
    let thiss = this
    let url = "//api.openparking.nl/parkingdata/summary/country/nl/?" + this.getParameters()
    fetch(url)
      .then(response => response.json())
      .then(json => {
        // json = thiss.handleFilters(json)
  ...
  handleFilters(json) {
    let url = "//api.openparking.nl/parkingdata/country/nl/?" + this.getParameters()
    let thiss = this
  ...
  onDezoom(val = false) {
    ...
    if (this.state.stackedTree.length > 0) {
      ...
      prev = temp.pop()
      let url = "//api.openparking.nl/parkingdata/" + prev + "/?" + this.getParameters()
      console.log(url)
